...
to setup
  ...
  spawn-carro-rua
  setup-gatos
  set mortes-por-fome 0
  set mortes-por-gato 0
  set mortes-por-carro 0
  reset-ticks
end
...
to spawn-carro-rua
  if random-float 1 < probabilidade-spawn-carro [  ; chance de spawn por tick
...
to mover-carros-rua
  ask carros-rua [
    ...
    let pomba-collide one-of pombos with [distance myself < 1.5]
    if pomba-collide != nobody [
      ask pomba-collide [ set mortes-por-carro mortes-por-carro + 1 die ]
    ]
    let gato-collide one-of gatos with [distance myself < 1.5]
    if gato-collide != nobody [
      ask gato-collide [ die ]  ;; mata o pombo
    ]
    ;; se sair da tela, morre
    if xcor >= max-pxcor - 1 or xcor <= min-pxcor + 1 [
      die
    ]
  ]
end
...
to carrinhos-soltam-comida
  ask carrinhos [
    if random-float 1 < taxa-de-comida [
      ask patch-here [
        sprout-comidas 1 [
          set shape "dot"
          set size 0.5
...
to movimento-dos-pombos
  ask pombos [
    ...
    ifelse gato-perto != nobody and distance gato-perto < 5 [
      ...
      set fome fome - 1
      if fome <= 0 [
        set mortes-por-fome mortes-por-fome + 1
        die
      ]
    ]
  ]
...
to reproduzir
  ask pombos with [sexo = "fêmea"] [
    ...
    if parceiro != nobody and random-float 1 < probabilidade-reproducao-pombos
    and fome > 25 and count pombos < 200 [
      hatch 1 [
        set sexo one-of ["macho" "fêmea"]
        set fome 25
      ]
...
to comportamento-dos-gatos
  ask gatos [
    let alvo one-of pombos with [distance myself < 6]
    ifelse alvo != nobody [
      face alvo
      fd velocidade-gato
      if distance alvo < 1 [
        ask alvo [ set mortes-por-gato mortes-por-gato + 1 die ]
...
to entrada-de-agentes
  ;; 1% de chance por tick de entrar 1 novo pombo
  if random-float 1 < 0.01 and count pombos < 200 [
    create-pombos 12 [
      set sexo one-of ["macho" "fêmea"]
      set shape "pombo"
      set size 1.5
      set color gray
      set fome 10 + random 5
      setxy random-xcor random-ycor
    ]
  ]
...
PENS
"default" 1.0 0 -955883 true "" "plot count comidas"
